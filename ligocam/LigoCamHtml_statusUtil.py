...
def ligocam_makehtml_status(puburl, pubhtml_dir, line, strcurUtcTime, curGpsTime):
    t = CamHtml.Table(header_row=[CamHtml.TableCell('Channel name', \
                ...
                width='6%', header=True), CamHtml.TableCell('DAQ<BR>failure?', \
                width='4%', header=True), CamHtml.TableCell('BLRMS<BR>change', \
                width='4%', header=True), CamHtml.TableCell('0.03-0.1', \
                width='4%', header=True), CamHtml.TableCell('0.1-0.3', \
                width='4%', header=True), CamHtml.TableCell('0.3-1', \
                width='4%', header=True), CamHtml.TableCell('1-3',width='4%', \
                ...
                header=True), CamHtml.TableCell('Image', width='4%', \
                header=True)])
    ...
    f.write('<p align="center" style="background-color:white;color:black;\
                        font-size:20px;margin-top:1px;margin-bottom:1px;">\n')
    f.write('Epoch: %s </p>\n' % strcurUtcTime)
    f.write('                      <table class="">\n')
    f.write('<thead>\n')
    f.write(htmlcode)
    ...
    f.write('      <script src="js/jquery.ba-throttle-debounce.min.js"> \
                                                            </script>\n')
