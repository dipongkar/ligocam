...
HTMLFILE = run_dir + 'pages/LigoCamHTML_sorted_2_'+str(options.curGpsTime)+'.html'
#HTMLFILE = run_dir + 'pages/LigoCamHTML_sorted'+str(options.curGpsTime)+'.html'
f = open(HTMLFILE, 'w')

t = CamHtml.Table(header_row=[CamHtml.TableCell('Channel name',width='29%',header=True), CamHtml.TableCell('Channel<BR>info',width='4%',header=True), CamHtml.TableCell('Image',width='5%',header=True), CamHtml.TableCell('Status',width='4%',header=True), CamHtml.TableCell('Disconnected?',width='6%',header=True), CamHtml.TableCell('DAQ<BR>failure?',width='4%',header=True), CamHtml.TableCell('BLRMS<BR>change',width='4%',header=True), CamHtml.TableCell('0.03-0.1',width='4%',header=True), CamHtml.TableCell('0.1-0.3',width='4%',header=True), CamHtml.TableCell('0.3-1',width='4%',header=True),CamHtml.TableCell ('1-3',width='4%',header=True), CamHtml.TableCell('3-10',width='4%',header=True), CamHtml.TableCell('10-30',width='4%',header=True), CamHtml.TableCell('30-100',width='4%',header=True), CamHtml.TableCell('100-<BR>300',width='4%',header=True), CamHtml.TableCell('300-<BR>1000',width='4%',header=True), CamHtml.TableCell('1000-<BR>3000',width='4%',header=True), CamHtml.TableCell('3000-<BR>10000',width='4%',header=True)])

thd1g = 1000; thd1l = 0.002
thd2g = 50; thd2l = 0.2

ff = open(run_dir + "results/Result_sorted_2_"+str(options.curGpsTime)+".txt","r")
#ff = open(run_dir + "results/Result_sorted"+str(options.curGpsTime)+".txt","r")
...
#    lines = ff.readlines()
    ...
    image = ('<a href="https://ldas-jobs.ligo-wa.caltech.edu/~dtalukder/Projects/detchar/LigoCAM/PEM/images/ASD/%s/%s" target="_blank">ASD</a>, <a href="https://ldas-jobs.ligo-wa.caltech.edu/~dtalukder/Projects/detchar/LigoCAM/PEM/images/TS/%s/%s" target="_blank">TS</a>' %(options.curGpsTime,chan_rn+'.png',options.curGpsTime,chan_rn+'.png'))
    image = CamHtml.TableCell(image,bgcolor='white',width='5%')
    info = ('<a href="http://pem.ligo.org/channelinfo/index.php?channelname=%s" target="_blank">link</a>' %(chan_rstrip))
    info = CamHtml.TableCell(info,bgcolor='white',width='4%')
    chan = CamHtml.TableCell(chan,bgcolor='white',width='29%')
    t.rows.append([chan, info, image, status, disconnect, comb, excess, band1, band2, band3, band4, band5, band6, band7, band8, band9, band10, band11])
...
f.write('	        </div>\n')
f.write('<p align="center" style="background-color:white;color:black;font-size:20px;margin-top:4px;margin-bottom:4px;">\n')
f.write('Epoch: %s </p>\n' %strcurUtcTime)
#f.write('<p align="left" style="background-color:white;color:gray;font-size:18px;margin-top:2px;margin-bottom:-40px;">BLRMS col. = ASD_tot_cur / ASD_tot_ref; alert if > 1000 or <1/500 for 0.03-1Hz and > 50 or < 1/5 for > 1Hz; disconnect if <= 0.2 counts and DAQ failure if <= 1e-8 counts/sqrt(Hz) for 10-100Hz (exception: no alert for ACC and MIC below 10Hz and SEIS above 30Hz; SEIS disconnect and DAQ failure defined for 3-30Hz.) </p>\n')
f.write('                      <table class="">\n')
f.write('<thead>\n')
f.write(htmlcode)
f.write('\n 	        	</table>\n\n')
...
HTMLFILE_pages = pubhtml_dir + 'pages/LigoCamHTML_'+str(options.curGpsTime)+'.html'
